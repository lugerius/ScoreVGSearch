		...
		if ( multi[0] && multi[0].length > 2 ) {	// Busco si hay un elemento de busqueda y a partir de dos caracteres inicia busqueda 
			...
			$.getJSON( dbase, function( data ) {	// Muestra resultados de búsqueda
				$.each ( data , function(key, val){
					...
					var track 	    = val["5"].toLowerCase();
					var year 		= val["6"].toLowerCase();
					var creator 	= val["7"].toLowerCase();
					var publish		= val["8"].toLowerCase();
					var trackNum	= val["10"];
					var toPlist     = 'Ep'+episode+' '+question.toUpperCase()+' '+creator+' '+year;
					$.each(multi, function(index, value){ // patrones de busqueda 
						...
						if (query.length > 2) {
							if (checkTopics(topic, question) || year.includes(query) || episode.toLowerCase().includes(query) || checkTopics(topic, track) || checkTopics(topic, creator) || checkTopics(topic, publish) || query == episodestr[0]+episode || query == episodestr[1]+episode || query == episodestr[2]+episode || query == episodestr[3]+episode) {
								var dataresult = {uri, time, end, toPlist};
								if(!allresults.find(obj => obj.uri+obj.time == uri+time)){ // elimino resultados repetidos
									results++;
									html += '<li><a href="#" class="alltext" onclick="setPlayer(\''+uri+'\', \''+time+'\', \''+end+'\', \''+toPlist+'\');">'+results+' Ep'+episode+'.'+trackNum+' - '+question.toUpperCase()+' ' + track + ' ' + year + ' <span class="ui-li-count">'+duration+'s</span></a><a href="#AddPlaylist" data-rel="popup" data-transition="slide" onclick="setPlaylist(\''+uri+'\', \''+time+'\', \''+end+'\', \''+toPlist+'\');">Agregar a Playlist</a></li>';
...
function setPlayer (uri, time, end, toPlist) {
	startP = 0; 
	cancelStop();
	stopVideo();
...
var playlist = [];
var playlistdata = [];
function setPlaylist (uri, time, end, toPlist) {
	...
	$("#playlist").show();
	$("tbody").append('<tr id="line'+renglon+'"><th>'+renglon+'</th><td><a href="#" onclick="startPlaylist(\''+renglon+'\');">'+toPlist+'</a></td><td>'+duration+'[s]</td></tr>');
	var fadepop = setTimeout(function(){ $("#AddPlaylist").popup("close"); }, 800);
	playlist.push(uri);
	playlistdata.push({"uri": uri, "time": time, "end": end, "question": toPlist});
	showDuration();
}
...
function setAllToPlist () {
	...
	$.each( jsonobject, function( key, value ) {
		setPlaylist(value["uri"], value["time"], value["end"], value["toPlist"]);
	});
	$("#autocomplete").fadeOut(200);
}
...
function onYouTubeIframeAPIReady() {
	var jsonFile = 'js/'+tipocue+'.json?nocache=' + (new Date()).getDay(); // recarga el archivo json cada día
	var lastEpisode = "";
